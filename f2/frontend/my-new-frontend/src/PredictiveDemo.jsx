...
export default function PredictiveDemo() {
  ...
  const [slideIndex, setSlideIndex] = useState(0);  // Start at index 0
  const [history, setHistory] = useState([         // Initialize with base history state
    {
      state: {
        brightness: 100,
        contrast: 100,
  ...
  const handleFileChange = (e) => {
    ...
    img.onload = () => {
      ...
      // Attach the base image to the latest history entry so predictive_core can find it
      setHistory((prev) => {
  ...
  const handleRunPredictive = () => {
    ...
    const newState = {
      ...
      brightness: 50,  // Apply brightness change
      contrast: 80,    // Apply contrast change
      saturation: 100,
      blur: 0,
      rotation: 0,
      flipH: false,
